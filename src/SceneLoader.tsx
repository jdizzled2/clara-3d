...
import { Coordinate } from "babylonjs";
...
export class SceneLoader {
  ...
  initCanvas(canvas: HTMLCanvasElement) {
    ...
    var createScene = () => {
      ...
      var camera = new BABYLON.ArcRotateCamera("camera", 0, BABYLON.Tools.ToRadians(25), 30,
                                               new BABYLON.Vector3(11, -2, 13.5), scene);
      ...
      for (let columnIndex = 0; columnIndex < columns; columnIndex++) {
        ...
        if (existingTile) {
          ...
          // existing tile is game object (clara, leaf, ghost), place lighter grass
                   ...
                   existingTile.tid == 14 ||
                   existingTile.tid == 15) {
                BABYLON.SceneLoader.ImportMesh("", "./", "GrassTop2.glb", this.scene, function (meshes) {
                  meshes.forEach((mesh) => {
                    if (mesh.material) {
                      mesh.material.needDepthPrePass = true;
                    }
  ...
  getTree(xcoord, zoord) {
    ...
    BABYLON.SceneLoader.ImportMesh("", "./", tree + '.glb', this.scene, function (meshes) {
      meshes.forEach((mesh) => {
        if (mesh.material) {
          mesh.material.needDepthPrePass = true;
        }
